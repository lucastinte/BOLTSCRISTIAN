  ...
  return (
    <section className="py-16 px-4 bg-black relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        {showLogo && (
          <div className="flex justify-center mb-10 animate-fade-in">
            <img
              ...
              className="w-40 md:w-56 h-auto drop-shadow-[0_0_15px_rgba(220, 38, 38,0.3)]"
            />
          </div>
        )}
        ...
        <div className="text-center mb-10">
          <Link to="/black-training" className="inline-block group cursor-pointer">
            <h2 className="text-4xl md:text-5xl font-bold text-white mb-2 transition-transform duration-300 group-hover:scale-105">
              Este es nuestro <span className="text-red-600 text-glow-red">Gimnasio</span>
            </h2>
        ...
        <div
          className="relative group h-[400px] md:h-[600px] rounded-3xl overflow-hidden border border-white/10 bg-zinc-900/50 box-glow-red"
        ...
        >
            ...
            {sortedImages.map((src, idx) => (
              <div key={idx} className="w-full h-full flex-shrink-0 relative">
                ...
                {idx < featuredNames.length && (
                  <div className="absolute bottom-8 left-8 bg-black/40 backdrop-blur-md border border-white/10 px-4 py-2 rounded-xl text-xs font-semibold text-red-600 uppercase tracking-widest opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500">
                    Vista Reciente
                  </div>
                )}
              </div>
          ...
          <button
            onClick={prevSlide}
            className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center rounded-full bg-black/30 backdrop-blur-md border border-white/10 text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600 hover:text-black hover:border-red-600"
          >
            <ChevronLeft className="w-6 h-6" />
          </button>
          <button
            onClick={nextSlide}
            className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center rounded-full bg-black/30 backdrop-blur-md border border-white/10 text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600 hover:text-black hover:border-red-600"
          >
