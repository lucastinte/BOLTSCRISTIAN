...
interface GymSectionProps {
  showLogo?: boolean;
  title?: string;
  gymType?: "arias" | "evolution" | "atenas" | "all";
  id?: string;
}
...
export function GymSection({ showLogo = false, title = "Nuestro Gimnasio", gymType = "all", id = "gym-gallery" }: GymSectionProps) {
  ...
  const featuredNames = ["0_triunvirato_logo.png", "2_evolution_gym_logo.png", "3_atenas_logo.jpg"];
  ...
  const sortedImages = Object.entries(gymImagesGlob)
    .filter(([path]) => {
      ...
      if (gymType === "atenas") return name.includes("atenas") || name.startsWith("3_");
      return false;
    })
    .sort(([pathA], [pathB]) => {
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        {showLogo && (
          <div className="flex justify-center mb-10 animate-fade-in">
            <Logo className="h-16 md:h-20 w-auto text-white drop-shadow-[0_0_15px_rgba(255, 255, 255,0.3)]" />
          </div>
        )}
        ...
        <div
          className="relative group h-[400px] md:h-[600px] rounded-3xl overflow-hidden border border-white/10 bg-zinc-900/50 box-glow-white"
        ...
        >
          ...
          >
            {sortedImages.map((src, idx) => {
              ...
              if (fileName.includes('atenas') || fileName.startsWith('3_')) sedeLabel = "Atenas by CWLife";
              ...
              return (
                <div key={idx} className="w-full h-full flex-shrink-0 relative">
                  ...
                  {/* Sede Label Banner */}
                  <div className="absolute top-8 left-8 bg-white text-black px-4 py-1.5 rounded-full text-[10px] font-black tracking-[0.2em] uppercase shadow-lg shadow-white/20 transform -rotate-1 group-hover:rotate-0 transition-all duration-500">
                    {sedeLabel}
                  </div>
          ...
          <button
            onClick={prevSlide}
            className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center rounded-full bg-black/30 backdrop-blur-md border border-white/10 text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white hover:text-black hover:border-white"
          >
            <ChevronLeft className="w-6 h-6" />
          </button>
          <button
            onClick={nextSlide}
            className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center rounded-full bg-black/30 backdrop-blur-md border border-white/10 text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white hover:text-black hover:border-white"
          >
