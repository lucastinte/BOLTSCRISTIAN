        ...
        <section className="py-20 px-4 bg-black relative overflow-hidden">
            ...
            <div className="max-w-6xl mx-auto relative z-10">
                <div className="text-center mb-16 relative">
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-white/10 blur-[100px] rounded-full animate-pulse-glow"></div>
                    <h1 className="text-5xl md:text-8xl font-black text-white mb-6 uppercase tracking-tighter relative z-10 italic">
                        CW <span className="text-white text-glow-white">LIFE</span>
                    </h1>
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-card border-white/20 mb-8 relative z-10">
                        <span className="text-[10px] text-white font-black uppercase tracking-[0.3em]">El Ataque Final</span>
                    </div>
                    <h2 className="text-3xl md:text-5xl font-black text-white mb-6 uppercase tracking-tighter relative z-10">
                        EL CIRCUITO <span className="text-white">REDUCTOR</span>
                    </h2>
                    <p className="text-gray-400 max-w-2xl mx-auto text-lg leading-relaxed relative z-10 font-medium">
                        ...
                        donde el ejercicio solo no llega. La unión de <span className="text-white font-black">ENTRENAMIENTO DE ÉLITE</span> +
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {features.map((feature, idx) => (
                        <div
                            key={idx}
                            className="group relative bg-zinc-900/50 backdrop-blur-sm border border-white/5 p-8 rounded-3xl hover:border-white/50 transition-all duration-500 hover:-translate-y-2"
                        >
                            <div className="mb-6 p-3 bg-black/50 rounded-2xl w-fit group-hover:scale-110 transition-transform duration-500 shadow-[0_0_15px_rgba(255, 255, 255,0.1)]">
                                {feature.icon}
                            </div>
                    ...
                    {showLogo && (
                        <div className="flex justify-center mb-16 animate-fade-in">
                            <div className="relative group">
                                <div className="absolute -inset-12 bg-white/10 blur-[60px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
                                <Logo className="h-20 md:h-28 w-auto text-white relative z-10 drop-shadow-[0_0_30px_rgba(255, 255, 255,0.2)] hover:scale-105 transition-all duration-500 filter drop-shadow-[0_0_10px_rgba(255,255,255,0.1)] group-hover:drop-shadow-[0_0_40px_rgba(255, 255, 255,0.3)]" />
                            </div>
                        </div>
                    )}
                    <button className="px-12 py-5 bg-white rounded-full text-black font-black uppercase tracking-[0.2em] text-xs hover:bg-gray-200 transition-all duration-300 shadow-[0_15px_35px_rgba(255, 255, 255,0.2)] transform hover:scale-105 active:scale-95">
                        UNIFICAR LA GUERRA
                    </button>
                </div>
            </div>
